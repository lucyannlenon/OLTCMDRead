...
final readonly class VSolEponFeatureAdapter extends AbstractOltFeatureProvider implements
    ...
    OnuMacDiscoveryInterface
{
    ...
    {
        $status = (new OnuStatusCommand($this->connection))->execute($registrationId);
        return $status === null
    ...
    {
        $status = (new OnuStatusCommand($this->connection))->execute($onu->registrationId);
        return $status === null
            ...
            : OltFeatureResult::supported(
                ...
                new OnuOperationalStatus(
                    $status->status,
                    $this->integer($status->distance),
                    $this->duration($status->aliveTime)
                )
            );
    }
    ...
    {
        if (preg_match('/(?:(\d+)\s*d)?\s*(\d+):(\d+):(\d+)/i', $value, $matches) !== 1) {
            return null;
        }
